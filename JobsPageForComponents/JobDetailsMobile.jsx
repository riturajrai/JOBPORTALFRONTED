...
const JobDetailsMobile = ({
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      {/* Sticky Header */}
      <header className="fixed top-0 left-0 right-0 bg-white shadow-md p-4 z-50 border-b border-gray-200">
          ...
          <div className="flex gap-3">
            <button
              ...
              className={`p-2 rounded-full transition-colors ${
                isSaved ? "bg-teal-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
              }`}
            ...
            <div className="relative">
              ...
              {shareTooltip && (
                <div className="absolute top-12 right-0 bg-gray-800 text-white text-xs rounded px-2 py-1 shadow-lg animate-fade">
                  Link copied!
                </div>
              )}
            </div>
          ...
          <div className="lg:flex-1">
            ...
            <section className="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
              <div className="flex justify-between items-start gap-4">
                <div>
                  <h1 className="text-2xl font-bold text-gray-900 mb-1">{job.title || "Job Title"}</h1>
                  <p className="text-lg text-gray-600 mb-4">{job.company || "Company Name"}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-700">
                ...
                {(job.salary_min || job.salary_max) && (
                  <div className="flex items-center gap-2">
                    <CurrencyDollarIcon className="w-5 h-5 text-teal-600 flex-shrink-0" />
                    <span>
                      {job.salary_min && job.salary_max
                ...
                {job.application_deadline && (
                  <div className="flex items-center gap-2">
                    <TimeIcon className="w-5 h-5 text-teal-600 flex-shrink-0" />
                    <span>Apply by: {new Date(job.application_deadline).toLocaleDateString()}</span>
                  </div>
                )}
                {job.job_priority && (
                  <div className="flex items-center gap-2">
                    <ExclamationCircleIcon className="w-5 h-5 text-teal-600 flex-shrink-0" />
            ...
            <section className="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                {job.skills && (
                  <div className="flex items-start gap-3">
                    <TagIcon className="w-5 h-5 text-teal-600 flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="font-medium text-gray-800">Skills</p>
                      <p className="text-gray-700">{Array.isArray(job.skills) ? job.skills.join(", ") : job.skills}</p>
                    </div>
                  </div>
                )}
                {job.work_location && (
                  <div className="flex items-start gap-3">
                    <BriefcaseIcon className="w-5 h-5 text-teal-600 flex-shrink-0 mt-0.5" />
                    <div>
                ...
                {job.benefits && (
                  <div className="flex items-start gap-3">
                    <HeartIcon className="w-5 h-5 text-teal-600 flex-shrink-0 mt-0.5" />
                    <div>
                ...
                {job.requirements && (
                  <div className="flex items-start gap-3">
                    <EyeIcon className="w-5 h-5 text-teal-600 flex-shrink-0 mt-0.5" />
                    <div>
                ...
                {job.preferred_languages && (
                  <div className="flex items-start gap-3">
                    <LanguageIcon className="w-5 h-5 text-teal-600 flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="font-medium text-gray-800">Languages</p>
                      <p className="text-gray-700">{job.preferred_languages}</p>
                    </div>
                  </div>
                )}
                {job.certifications && (
                  <div className="flex items-start gap-3">
                    <CertificateIcon className="w-5 h-5 text-teal-600 flex-shrink-0 mt-0.5" />
                    <div>
          ...
          <div className="lg:w-80 xl:w-96 flex-shrink-0">
            ...
            <section className="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
              ...
              <div className="space-y-4">
                {job.company_size && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-700">{job.company_size}</p>
                    </div>
                  </div>
                )}
                {job.category && (
                ...
                {job.apply_url && (
                  <div className="flex items-start gap-3">
                    <LinkIcon className="w-5 h-5 text-teal-600 flex-shrink-0 mt-0.5" />
                    <div>
                      <p className="font-medium text-gray-800">Apply Here</p>
                      <a
                        href={job.apply_url}
                        target="_blank"
            ...
            {/* Report Button */}
            {!isReported && (
              <section className="bg-white rounded-xl shadow-sm p-6 mb-6 text-center border border-gray-100">
                <button
                  onClick={handleShowReportModal}
                  className="flex items-center justify-center gap-2 text-gray-600 hover:text-teal-600 font-medium transition-colors mx-auto"
                >
            ...
            {!showQuestions && (
              <section className="hidden lg:block bg-white rounded-xl shadow-sm p-6 sticky top-20 border border-gray-100">
                <button
                  onClick={handleApply}
        ...
        <AnimatePresence>
          {actionFeedback && (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => actionFeedback.onDismiss?.()}
          ...
          {error && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              className="fixed top-20 left-1/2 transform -translate-x-1/2 max-w-md w-full p-4 rounded-lg shadow-lg text-sm z-50 bg-red-50 text-red-800 border border-red-200"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <svg className="w-5 h-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="bg-white rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-xl">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-semibold text-gray-900">
                  Application Questions ({currentQuestionIndex + 1}/{questions.length})
                </h3>
                <button
